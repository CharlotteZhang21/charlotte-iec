...
export function scaleThenFade(sprite, delay, duration) {
    ...
    return sprite.game.add.tween(sprite).to({ alpha: 0 }, fadeDuration, Phaser.Easing.Linear.None, true, duration - scaleDuration);
...
// animation to display an item appear from the ground - height 0 - 100, position up and down to the ground
export function appearFromGround(sprite, delay, duration, easing, cb) {

    var initialScale = sprite.scale.y;

    sprite.scale.y = 0.001;
    
    return sprite.game.add.tween(sprite.scale).to({
            y: [initialScale * 1.2, initialScale]
        }, duration, easing, true, delay);
...
export function appearFromSky(sprite, delay, duration, easing, cb) {
    ...
    this.scaleIn(sprite, delay, duration/2, easing, function(sprite){
        sprite.game.add.tween(sprite).to({
            y: [initialY, initialY * 0.9, initialY]
...
export function jiggleJump(sprite, delay, duration, easing, cb) {
    var initialScale = sprite.scale.y;
    ...
    sprite.game.add.tween(sprite.scale).to({
        ...
        easing,
        true,
        delay, -1).yoyo(true, 800);
}

export function disappearToSide(sprite, xOffset, delay, duration, easing, cb) {
    var tween = sprite.game.add.tween(sprite).to({
        x: xOffset,
        alpha: 0,
    }, duration, easing, true, delay)
    ...
    if (cb)
        tween.onComplete.add(cb(sprite), this);

    return tween;
}

export function slightMoveYoyo(sprite, xOffset, yOffset, delay, duration, easing) {
    var initialX = sprite.x,
        initialY = sprite.y;

    var targetX = initialX - xOffset,
        targetY = initialY - yOffset;

    return sprite.game.add.tween(sprite).to({
        x: [targetX, initialX * 0.9, initialX],
        y: [targetY, initialY * 0.9, initialY],
    }, duration, easing, true, delay)
}