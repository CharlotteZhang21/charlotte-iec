...
(function(root, factory) {
    ...
    */
    ...
    var setTransition = function(pre, cur, next, preTransition, curTransition, nextTransition) {
        ...
        //add/remove hide-item class based on whether the carousel image is currently visible
        if (pre.classList)
		  pre.classList.add('hide-item');
		else
		  pre.className += ' ' + 'hide-item';
		  
		
		if (cur.classList)
		  cur.classList.remove('hide-item');
		else
		  cur.className = cur.className.replace(new RegExp('(^|\\b)' + 'hide-item'.split(' ').join('|') + '(\\b|$)', 'gi'), ' ');
				
		  
		if (next.classList)
		  next.classList.add('hide-item');
		else
		  next.className += ' ' + 'hide-item';
    ...
    var prepareIndicator = function(slider, wrapClassName, className, howMany, activeIndex, activeClass) {
        ...
        slider.container.parentNode.appendChild(indicatorWrap);
